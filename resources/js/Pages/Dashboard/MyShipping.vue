<template>
	...
	<div class="tab-content dashboard-content mt-all-40">
    	<div id="hotels" class="tab-pane fade show active">
   				<div class="row">
   					<div class="col-md-12">
   						<h3>Mis Envíos pendientes por llevar</h3>	
   					</div>
    			</div>
    				...
    				<table class="table table-striped table-bordered table-hover">
                		...
                		<tbody>
                    		<tr v-for="order in orders.data" :key="order.id">
                    			...
                    			<td>
                    				<a class="text-info links-orders" href="javascript:void(0);" @click.prevent="sending(order.id)">Marcar como enviado</a>
                    			</td>
                    		</tr>
                		</tbody>
            		</table>
    	...
    	methods: {
        	sending(order){
        		this.$swal({
  					...
  					confirmButtonText: 'Si, marcar como enviado!',
  					cancelButtonText: 'Cancelar',
				}).then((result) => {
  						if (result.isConfirmed) {
  							this.$inertia.get(route('shipping.finish',{order : order}),
  							{
								preserveScroll: true,
  							})
  						}
